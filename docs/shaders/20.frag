...
void main( void ) {
    ...
    for (float i = 0.; i < 10.; i++) {
        if (i >= n) { break; }
        vec2 iStart = rotate(start, r * 1.5);
        vec2 iCenter = center + rotate(start * size, i * 2. * PI / n) * s * 1.2;
        float iSize = size * z;
        c += polygonLine(p - iCenter, n, iSize, iStart);

        for (float j = 0.; j < 10.; j++) {
            if (j >= n) { break; }
            vec2 jStart = rotate(iStart, r * 4.);
            vec2 jCenter = iCenter + rotate(iStart * iSize, j * 2. * PI / n) * s * 1.2;
            float jSize = iSize * z;
